...
/*
The project can be set up by following the usual Workshop 2 set up. Set up Workshop 2 as the startup project and build the solution. The solution will then need to be run.
For different modes and to use different images you will need to go to the ‘Debug’ tab and click on ‘Tutorial 2 Properties’, inside of that menu click on another tab called ‘Debugging’ and type your command arguments into the ‘Command Arguments’ box. For the list of arguments to put in type ‘-h’. To test an unseen Monochrome image type ‘-f MC_test1.ppm’ and run the project. This will enhance the contrast on the image very well.
The implementation can be run on colour images and on both 8bit and 16 bit images. Histogram based on local memory has been implemented and so has the Hillis and Steele scan and the Blelloch scan as mentioned in the workshops. Images in the folder that have been tested are ‘test.ppm’, ‘test_large.ppm’, ‘MC_test1.ppm’, ‘MC_test2.ppm’, ‘Colour_test1.ppm’ and ‘Colour_test2.ppm’.
The kernels that have been implemented are the basic histogram, Cumulative histogram, histogram using local memory, C-hist using HS scan, helper kernel to obtain block sums, an exclusive scan, an exclusive scan using Blelloch, a complete histogram, obtaining the look up tables and kernels to output the images.
*/
...
int main(int argc, char** argv)
{
	...
	try
	{
		...
		//the aim is to ensure that the global size is a multiple of the local size for the padding 
		...
		//This does the same but for the 16bit image size
		...
		//aiming for the global size to be a multiple of the local size for the padding
		...
		std::cout << "Memory transfer time: " << total_upload_time / 1000 << std::endl;
		std::cout << "---------------------------------------------------------" << std::endl;
		std::cout << "Kernel execution time: " << total_kernel_time / 1000 << std::endl;
		std::cout << "---------------------------------------------------------" << std::endl;
		std::cout << "   Histogram kernel execution time: " << kernel1_time / 1000 << std::endl;
		std::cout << "---------------------------------------------------------" << std::endl;
		std::cout << "   Cumulative histogram kernel execution time: " << kernel2_time / 1000 << std::endl;
		std::cout << "---------------------------------------------------------" << std::endl;
		std::cout << "Program execution time: " << (total_upload_time + total_kernel_time + output_image_download_time) / 1000 << std::endl;
