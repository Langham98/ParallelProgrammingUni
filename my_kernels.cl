...
//8but image hist with specified bins
...
kernel void get_chist_HS(global const uint* H, global uint* CH, local uint* H_local, local uint* CH_local)
{
	...
	H_local[local_id] = H[global_id]; //cache hist values from global to local
